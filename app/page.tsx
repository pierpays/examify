...
const features = [
  ...
  ["Practice exams", "Find exams by teacher, title, date or Examtify code and track performance over time."],
  ["Verified institutions", "Institution accounts are reviewed by Examtify before receiving full access."],
  ...
  ["Real achievements", "Students can choose to share achievements backed by completed Examtify attempts."],
];
...
export default function HomePage() {
  return (
    <main className="min-h-screen bg-[#F8FAFC] text-[#0F172A]">
      ...
      <section className="overflow-hidden border-b border-slate-200 bg-[radial-gradient(circle_at_top_right,_#ede9fe_0,_transparent_32%),radial-gradient(circle_at_top_left,_#dbeafe_0,_transparent_34%),linear-gradient(to_bottom,#ffffff,#f8fafc)]">
        <div className="mx-auto grid max-w-6xl gap-12 px-4 py-16 sm:px-6 sm:py-24 lg:grid-cols-[1.05fr_.95fr] lg:items-center">
          <div>
            ...
            <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-600">Examtify brings students, teachers, parents and verified institutions together to practice, share knowledge, discover resources and celebrate progress.</p>
            ...
            <p className="mt-4 text-sm text-slate-500">Institution accounts are verified by Examtify before full access is granted.</p>
          </div>
          ...
          <div className="relative rounded-[2rem] bg-gradient-to-br from-[#1E3A8A] via-[#2563EB] to-[#7C3AED] p-6 text-white shadow-2xl shadow-blue-200 sm:p-8">
            ...
            <div className="relative flex items-center gap-4"><div className="rounded-2xl bg-white p-2"><ExamifyMark className="h-14 w-14" /></div><div><p className="text-sm font-bold text-blue-100">EXAMTIFY COMMUNITY</p><h2 className="text-2xl font-extrabold">Your academic journey, together.</h2></div></div>
            <div className="relative mt-8 space-y-4">
              <div className="rounded-2xl bg-white/10 p-4 ring-1 ring-white/20"><p className="text-xs font-bold text-blue-100">TEACHER UPDATE</p><p className="mt-2 font-semibold">A teacher published a new practice exam.</p><div className="mt-3 rounded-xl bg-white p-3 text-sm font-semibold text-[#0F172A]">Algebra Foundations · MATH-1024</div></div>
      ...
      <section className="mx-auto max-w-6xl px-4 py-16 sm:px-6 sm:py-20"><p className="text-sm font-bold text-[#2563EB]">FOR THE WHOLE ACADEMIC COMMUNITY</p><h2 className="mt-2 text-3xl font-extrabold tracking-tight sm:text-4xl">Choose how you use Examtify.</h2><div className="mt-8 grid gap-4 sm:grid-cols-2">{roles.map(([title,description,href,label])=><article key={title} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-0.5 hover:border-blue-200 hover:shadow-md"><h3 className="text-xl font-bold">{title}</h3><p className="mt-2 text-sm leading-6 text-slate-600">{description}</p><Link href={href} className="mt-5 inline-flex text-sm font-bold text-[#2563EB]">{label} →</Link></article>)}</div></section>
      ...
      <footer className="border-t border-slate-200 bg-white">
        <div className="mx-auto flex max-w-6xl flex-col gap-5 px-4 py-7 sm:px-6 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs font-semibold text-slate-500">
            ...
            <span>© {new Date().getFullYear()} Examtify</span>
          </div>
        </div>
      </footer>
    </main>
